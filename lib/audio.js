...
let   pendingTimers = [];     // deferred plays that have not started yet
...
function stopAudio() {
  generation++;
  cancelCurrent();
  // speakFallback goes through speechSynthesis, which none of the above touches.
  if (typeof speechSynthesis !== 'undefined') speechSynthesis.cancel();
}

/*
 * Run fn after delayMs — cancelled by stopAudio, like everything else.
 *
 * A raw setTimeout(() => playAudio(...)) is invisible to stopAudio: the timer
 * survives the thing that scheduled it and starts fresh audio on a question or
 * screen that no longer shows the thing speaking. That is how 课4's t+u blend
 * kept narrating over the next question. Every deferred play goes through here.
 */
function playLater(delayMs, fn) {
