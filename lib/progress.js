...
function checkProgress(result) {
  ...
  const cleared = total > 0 && (result.correct / total) >= CLEAR_RATIO;

  if (!cleared) {
    return { lesson: lesson, cleared: null, unlocked: null, next: next, alreadyCleared: already };
  }
  ...
  return { lesson: lesson, cleared: lesson, unlocked: unlocked, next: next, alreadyCleared: already };
}
