...
function rubyEl(hanzi, pinyin) {
  ...
  // Only 汉字 get an annotation, and only they consume a syllable. A line
  // like 「见了 j q x」 mixes characters with bare pinyin letters, and the
  // letters must be left alone rather than throwing the pairing out.
  const cjkCount = chars.filter(ch => CJK.test(ch)).length;
  if (!syls.length || syls.length !== cjkCount) {
