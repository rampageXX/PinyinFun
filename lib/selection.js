...
/* Letters she has never been asked, first; the rest after, order preserved. */
function untestedFirst(sounds, strengths) {
  const seen = sounds.filter(function (s) { return (strengths[s.id] || {}).attempts > 0; });
  const fresh = sounds.filter(function (s) { return !((strengths[s.id] || {}).attempts > 0); });
...
function pickTodaysItems(dateString, lesson) {
  ...
  return {
    // Every letter the lesson teaches, and today's before any review — with
    // the ones she has not been asked yet at the very front.
    //
    // They used to be capped at 4 and shuffled in with the review items, so a
    // letter the lesson exists to teach could go unasked. Ordering matters as
    // much as inclusion: a lesson teaching more than four letters cannot cover
    // them all in one mission, and the draw is seeded by date, so without
    // untested-first a second sitting on the same day would ask the identical
    // four and the remaining letters could never be reached at all.
    sounds: untestedFirst(seededShuffle(currentSounds, rng), strengths)
